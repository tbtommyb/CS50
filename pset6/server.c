...
{
    struct stat buffer;
    char* filePath = malloc(sizeof(char) * (strlen(path) + 12)); /* leave space for "/index.html" plus null */
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    do
    {
        buffer = (BYTE*) realloc(buffer, len + CHUNK);
        nchars = fread(&buffer[len], 1, CHUNK, file);
        len += nchars;
    }
    while(nchars == CHUNK);
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    if(sscanf(line, "%ms %ms %ms", &method, &target, &version) != 3)
    {
        error(400);
        return false;
    }
    ...
    // Split query string out if present
    char* absolute_path = strtok(target, "?");
